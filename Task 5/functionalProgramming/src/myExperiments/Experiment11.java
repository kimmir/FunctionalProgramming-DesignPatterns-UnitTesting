...
public class Experiment11 {
	class DriverStreamBuilder {		
		...
		public Stream<Driver> build() {
			Stream.Builder<Driver> builder = Stream.builder();
			table.forEach(driver -> builder.add(driver));
			return builder.build();
		}
	}
	
	public void run() {
		DriverStreamBuilder builder = new DriverStreamBuilder();
		Stream<Driver> driverStream = builder.build();
		driverStream.forEach(driver -> System.out.println(driver));
	}
