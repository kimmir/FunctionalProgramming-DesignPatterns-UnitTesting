...
public class Experiment12 {
	...
	public static void main(String[] args) {
		...
		Optional<Driver> drivers = table.stream().reduce((name1,name2)
				-> name1.getDriver().length() > name2.getDriver().length()
				? name1:name2);
		
		drivers.ifPresent(System.out::println);
		System.out.println();
		
		drivers = table.parallelStream().reduce((name1,name2)
				-> name1.getDriver().length() > name2.getDriver().length() 
				? name1:name2);
