...
public class Experiment3 {
	public static void main(String[] args) {
		...
		OptionalInt min = table.stream().mapToInt(Driver::getAge).min();
		if(min.isPresent()) {
			System.out.printf("Stream: Lowest Age is %d\n", min.getAsInt());
		} else {
			System.out.println("Stream: Min Failed");
		}
		
		min = table.parallelStream().mapToInt(Driver::getAge).min();
		if(min.isPresent()) {
			System.out.printf("Parallel Stream: Lowest Age is %d\n",min.getAsInt());
		} else {
			System.out.println("Parallel Stream: Min Failed");
		}
	}
}
