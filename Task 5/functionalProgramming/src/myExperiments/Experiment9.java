...
public class Experiment9 {
	...
	class ColourReplacer implements Function<Driver, String> {
		public String apply(Driver driver) {
			return driver.getCarColour().replaceAll("Silver", "Black");
		}
	}
	...
	public void run() {
		...
		table.stream().map(new ColourReplacer()).forEach(colour -> System.out.println(colour));
		System.out.println();
		table.parallelStream().map(new ColourReplacer()).forEach(colour -> System.out.println(colour));
